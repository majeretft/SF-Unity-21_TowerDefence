        ...
        private void Update()
        {
            if (_target)
            {
                ...
                else
                {
                    ...
                    {
                        Vector2 turretDirection = _target.transform.position - t.transform.position + (Vector3)_target.velocity * _lead;
                        t.transform.up = turretDirection;
                        t.Fire();
                    }
                }
            }
            else
            {
                ...
                if (collider)
                    _target = collider.transform.root.GetComponent<Rigidbody2D>();
            }
        }
